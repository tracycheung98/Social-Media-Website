...
export default function Search() {
    ...
    const handleSearch = async () => {
        ...
        if (results.length === 0) {
            ...
            navigate("/search", { state: { searchButtonClicked:true, searchResults: results } });

        } else {
            // setSearchButtonClicked(true);
            console.log('Users found.');
            setSearchResults(results);
            navigate("/Social-Media-Website/search", { state: {  searchButtonClicked:true, searchResults: results } });
        }
    };
